...
func TestPluginStopPolicyPreservesUnmatchedErrorsAndWireRegex(t *testing.T) {
	original := rpcPluginError{message: "gemini_web_omni:submission_unknown", statusCode: 502}
...
func TestPluginCredentialStopPolicyPreventsGenerationFailover(t *testing.T) {
	for _, stream := range []bool{false, true} {
		t.Run(fmt.Sprintf("stream=%v", stream), func(t *testing.T) {
			...
			upstreamErr := rpcPluginError{message: "gemini_web_omni:submission_unknown", statusCode: 502}
