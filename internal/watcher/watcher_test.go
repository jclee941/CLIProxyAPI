...
func TestPersistConfigAndAuthAsyncInvokePersister(t *testing.T) {
	store := &stubStore{cfgDone: make(chan struct{}, 1), authDone: make(chan struct{}, 1)}
	w := &Watcher{
	...
	for name, done := range map[string]<-chan struct{}{"config": store.cfgDone, "auth": store.authDone} {
		select {
		case <-done:
		case <-time.After(time.Second):
...
func TestScheduleConfigReloadDebounces(t *testing.T) {
	...
	var reloads int32
	store := &stubStore{cfgDone: make(chan struct{}, 2)}
	w := &Watcher{
		configPath:     cfgPath,
		authDir:        authDir,
		reloadCallback: func(*config.Config) { atomic.AddInt32(&reloads, 1) },
	...
	select {
	case <-store.cfgDone:
		t.Fatal("config reload was not debounced")
