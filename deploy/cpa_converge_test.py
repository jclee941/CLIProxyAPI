...
def host(test: unittest.TestCase) -> Host:
    ...
    for directory in (clone / "deploy/gw", clone / "deploy/cw", plugins / "gw", state, binaries):
    ...
    os.environ.update(PATH=f"{binaries}:{os.environ['PATH']}", FAKE_STATE=str(state), FAKE_PLUGINS=str(plugins), FAKE_NOW="2026-09-26 02:00:00")
    ...
    fixture = Host(plugins, state, commit, settings)
