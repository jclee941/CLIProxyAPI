	...
	for _, source := range []string{"openai", "openai-response", "claude", "gemini-cli", ""} {
		t.Run(source, func(t *testing.T) {
			...
			request := struct {
				...
				Body                []byte
			}{source, omniModel, []byte(`{"contents":[{"parts":[{"text":"test"}]}]}`)}

			result := invoke(t, service, "request.intercept_before", request)

			if !result.OK {
				t.Fatalf("host ignores interceptor errors: %+v", result.Error)
			}
			var response struct {
				Terminate       bool
				StatusCode      int
	...
	for _, models := range []struct {
		...
		t.Run(models.requested+models.actual, func(t *testing.T) {
			...
			request := struct {
				...
				Body                                []byte
			}{"openai", models.requested, models.actual, []byte(`{}`)}
			...
			if response.Terminate != models.terminate {
				t.Fatalf("wrong model scope: %s", result.Result)
			}
		})
	}
}
	...
	for _, scenario := range []struct {
		...
		t.Run(scenario.body, func(t *testing.T) {
			request := struct {
				...
				Stream                              bool
				Body                                []byte
			}{"gemini", omniModel + "(high)", omniModel, scenario.stream, []byte(scenario.body)}
			...
			if response.Terminate != scenario.terminate {
				t.Fatalf("wrong payload decision: %s", result.Result)
			}
		})
	}
}
		...
		for _, model := range []string{flashModel, "gemini-web-flash", "claude-sonnet", "gpt-5", ""} {
			for _, preceding := range []bool{true, false} {
				t.Run(method+model+string(rune('0'+map[bool]int{false: 0, true: 1}[preceding])), func(t *testing.T) {
					...
					}{"openai", model, body, headers, json.RawMessage(`{}`)}
					if !preceding {
						request.Body = []byte(`{}`)
						request.Headers = nil
					}
					...
					if err := json.Unmarshal(result.Result, &modifications); err != nil {
						t.Fatal(err)
					}
					if len(modifications) != 0 {
						t.Fatalf("interceptor rewrote another plugin's request: %s", result.Result)
					}
					if preceding && (!bytes.Equal(request.Body, body) || request.Headers.Get("X-Structured-Output") != "keep") {
						t.Fatal("previous interceptor changes lost")
					}
				})
			}
		}
